SELECT 'shell' AS component,
  '👌SQL Project' AS title,
  TRUE AS fixed_top_menu,
  'dark' AS theme,
  'golf' AS icon,
  JSON('[
  { "title": "Home", "link": "index.sql", "icon": "home" },
  { "title": "About", "link": "/about.sql", "icon": "info-square" },

  {
    "title": "Sales",
    "icon": "section",
    "submenu": [
      { "title": "Orders", "link": "t_orders.sql", "icon": "table" },
      { "title": "Purchases", "link": "t_purchases.sql", "icon": "table" },
      { "title": "Transactions", "link": "t_transactions.sql", "icon": "table" },
      { "title": "Order Status", "link": "t_ord_status.sql", "icon": "table" },
      { "title": "Deal Size", "link": "t_deal_size.sql", "icon": "table" }
    ]
  },

  {
    "title": "Product",
    "icon": "section",
    "submenu": [
      { "title": "Products", "link": "t_products.sql", "icon": "table" },
      { "title": "Product Line", "link": "t_products_line.sql", "icon": "table" },
      { "title": "Inventory", "link": "t_inventory.sql", "icon": "table" }
    ]
  },

  {
    "title": "Customer Info",
    "icon": "section",
    "submenu": [
      { "title": "Customer", "link": "t_customer.sql", "icon": "table" },
      { "title": "State", "link": "t_states.sql", "icon": "table" },
      { "title": "Country", "link": "t_country.sql", "icon": "table" },
      { "title": "City", "link": "t_city.sql", "icon": "table" }
    ]
  },
 { "title": "Raw Data", "link": "t_raw_data.sql", "icon": "section" }
]') AS menu_item,
     '' AS footer,
      'Poppins' as font,
     'en-US' as language,
      'Menu' as navbar_title,
      '' as search_value,
      --30 as refresh,
  TRUE AS sidebar;

SELECT 'divider' as component,
       'NEW PRODUCT' as contents;

SELECT 'csv' as component,
       'Product Line.csv' as filename,
       'md' as size,
       'Product Line' as title,
        'cyan' as  color,
       'file-download' as icon;

SELECT * FROM "Product Line";


SELECT 'form' AS component,
       'p_add_prod.sql' AS action,
        'red' as  color,
       'Check' AS validate;


SELECT 'file' as type,
        'product_new' as name,
        8 as width,
        'Upload a CSV file with Series, Model, Manfucturer Specied Retail Price, Product Line as columns' as description_md,
        '' as label,
        TRUE as required;

SELECT 'csv' as component,
       'Failed Products.csv' as filename,
       'md' as size,
       'Export failed' as title,
        'cyan' as color,
       'file-download' as icon
    WHERE EXISTS(SELECT 1 FROM new_product WHERE "Message" <>'Successful');

SELECT 
DISTINCT CONCAT("Series"||'_'||"Model") AS "Product Code",
"Manfucturer Specied Retail Price",
"Line Code",
"Product Line",
"Message"
 FROM new_product
WHERE "Message" <>'Successful';

SELECT 'table' as component,
   TRUE    as freeze_columns,
   TRUE    as freeze_headers,
   TRUE    as hover,
   TRUE    as striped_rows,
   TRUE    as sort,
   TRUE    as search,
  'table' as icon
  WHERE EXISTS(SELECT 1 FROM new_product);

SELECT
DISTINCT CONCAT("Series"||'_'||"Model") AS "Product Code","Manfucturer Specied Retail Price","Line Code","Product Line","Message"
 FROM new_product
WHERE EXISTS(SELECT 1 FROM new_product);

SELECT 'button' as component,
       'md' as size,
       'end' as justify;

SELECT 
       'Submit' as title,
       'Cyan' as  outline,
       'p_submit_new_prod.sql' as link
       WHERE EXISTS(
        SELECT 1 FROM new_product
         WHERE "Message" = 'Successful');



